"""
Service layer for Hero Images Mobile.
...
from app.schemas.hero_images_mobile import HeroImagesMobileCreate, HeroImagesMobileUpdate, HeroImagesMobileOut
from app.crud import hero_images_mobile as crud
from app.utils.gridfs import upload_image, replace_image, delete_image, _extract_file_id_from_url


async def create_item_service(category: str, idx: int, image: UploadFile = None) -> HeroImagesMobileOut:
    """
    ...
    try:
        ...
        payload = HeroImagesMobileCreate(category=category, idx=idx, image_url=url)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create HeroImagesMobile: {e}")


async def list_items_service(skip: int, limit: int, sort_by_idx: bool, category: Optional[str] = None) -> List[HeroImagesMobileOut]:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list HeroImagesMobile: {e}")


async def get_item_service(item_id: PyObjectId) -> HeroImagesMobileOut:
    """
    Get a single hero image mobile by ID.
    ...
    try:
        ...
        if not d:
            raise HTTPException(status_code=404, detail="HeroImagesMobile not found")
        return d
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to get HeroImagesMobile: {e}")
...
async def update_item_service(
    ...
    try:
        ...
        if "E11000" in str(e) and ("category" in str(e) or "idx" in str(e)):
            raise HTTPException(status_code=409, detail="Duplicate category+idx combination.")
        raise HTTPException(status_code=500, detail=f"Failed to update HeroImagesMobile: {e}")
...
async def delete_item_service(item_id: PyObjectId):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to delete HeroImagesMobile: {e}")
