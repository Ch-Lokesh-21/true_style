...
import type { Cards2, Cards2FormData } from '../types';
...
const FormModal: React.FC<FormModalProps> = ({ open, onClose, item, mode }) => {
  ...
  const createMutation = useCreateCards2();
  const updateMutation = useUpdateCards2();
  ...
  const onSubmit = async (data: Cards2FormData) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>
        {mode === 'view' ? 'View' : mode === 'edit' ? 'Edit' : 'Create'} Card 2
      </DialogTitle>
...
const DeleteModal: React.FC<DeleteModalProps> = ({ open, onClose, item }) => {
  const deleteMutation = useDeleteCards2();
...
export const Cards2Content: React.FC = () => {
  const { data, isLoading, error } = useCards2List();
  ...
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; item: Cards2 | null }>({
    open: false,
    item: null,
  });
  ...
  React.useEffect(() => {
    if (error) {
      console.error('Error loading cards 2:', error);
    }
  }, [error]);

  const columns: GridColDef<Cards2>[] = useMemo(
    () => [
