// HeroImagesMobileContent.tsx - Same as HeroImagesContent but for mobile
...
import {
  useHeroImagesMobileList,
  useCreateHeroImageMobile,
  useUpdateHeroImageMobile,
  useDeleteHeroImageMobile,
...
const FormModal: React.FC<FormModalProps> = ({ open, onClose, item, mode }) => {
  ...
  const createMutation = useCreateHeroImageMobile();
  const updateMutation = useUpdateHeroImageMobile();
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>
        {mode === 'view' ? 'View' : mode === 'edit' ? 'Edit' : 'Create'} Hero Image (Mobile)
      </DialogTitle>
...
const DeleteModal: React.FC<DeleteModalProps> = ({ open, onClose, item }) => {
  const deleteMutation = useDeleteHeroImageMobile();
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        Are you sure you want to delete this mobile hero image? This action cannot be undone.
      </DialogContent>
      <DialogActions>
...
export const HeroImagesMobileContent: React.FC = () => {
  const { data, isLoading, error } = useHeroImagesMobileList();
  ...
  React.useEffect(() => {
    if (error) {
      console.error('Error loading hero images mobile:', error);
    }
  }, [error]);
