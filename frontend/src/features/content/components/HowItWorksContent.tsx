...
import type { HowItWorks, HowItWorksFormData } from '../types';
...
const FormModal: React.FC<FormModalProps> = ({ open, onClose, item, mode }) => {
  ...
  const createMutation = useCreateHowItWorks();
  const updateMutation = useUpdateHowItWorks();
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>
        {mode === 'view' ? 'View' : mode === 'edit' ? 'Edit' : 'Create'} How It Works Item
      </DialogTitle>
...
const DeleteModal: React.FC<DeleteModalProps> = ({ open, onClose, item }) => {
  const deleteMutation = useDeleteHowItWorks();
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        Are you sure you want to delete this how it works item? This action cannot be undone.
      </DialogContent>
      <DialogActions>
...
export const HowItWorksContent: React.FC = () => {
  const { data, isLoading, error } = useHowItWorksList();
  ...
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; item: HowItWorks | null }>({
    open: false,
    item: null,
  });
  ...
  const columns: GridColDef<HowItWorks>[] = useMemo(
    () => [
