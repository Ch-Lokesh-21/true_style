...
const FormModal: React.FC<FormModalProps> = ({ open, onClose, item, mode }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>
        {mode === 'view' ? 'View' : mode === 'edit' ? 'Edit' : 'Create'} Policy
      </DialogTitle>
...
export const PoliciesContent: React.FC = () => {
  ...
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; item: Policy | null }>({
    open: false,
    item: null,
  });
  ...
  const columns: GridColDef<Policy>[] = useMemo(
    () => [
