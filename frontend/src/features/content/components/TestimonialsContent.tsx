...
const FormModal: React.FC<FormModalProps> = ({ open, onClose, item, mode }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>
        {mode === 'view' ? 'View' : mode === 'edit' ? 'Edit' : 'Create'} Testimonial
      </DialogTitle>
...
export const TestimonialsContent: React.FC = () => {
  const { data, isLoading, error } = useTestimonialsList();
  ...
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; item: Testimonial | null }>({
    open: false,
    item: null,
  });
  ...
  const columns: GridColDef<Testimonial>[] = useMemo(
    () => [
